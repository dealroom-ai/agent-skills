...
description: >-
  ...
  (`schema.json`, 23 tables — Grep it, never Read it whole), the entity model and join paths (`schema.md`), the default VC-funding
...
| Investor participation per round (who took part) | `funding_iu` / `vc_funding_iu` + `UNNEST(funding_investors)` | One array element per investor (`bobject_investor_id`, `flg_is_lead_investor`); join → entities.id. For *how much* each invested, use `vc_funding_breakdown_iu` |
| Per-investor amount invested per round (`bucket_usd`) | `vc_funding_breakdown_iu` | One row per investor×round; `bucket_usd` = lead-weighted estimated cheque; `flg_is_estimated` flags modelled round amounts. Bucket into $0–15M/$15–100M/$100M+ for source-of-capital-by-stage |
| Investor ownership / cap table (`ownership_pct`) | `vc_ownership_iu` | One row per holder×company; diluted `ownership_pct` (fraction 0–1). Estimated cap table, not filed |
| Round amount (disclosed-or-estimated) | `vc_funding_estimated_amounts_iu` | One row per round; use `amount_usd_filled` (disclosed else modelled median); `flg_is_estimated` flags estimates |
...
| Companies by main HQ region / region rankings | `main_hq_regions` | Match a company's HQ location ids → `dim_locations_iu_unique_id`, filter `source='curated'`, `GROUP BY main_hq_region` (see schema.md for the query) |
