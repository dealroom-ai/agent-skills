...
| Companies by main HQ region / region rankings | `main_hq_regions` | Match a company's HQ location ids → `dim_locations_iu_unique_id`, filter `source='curated'`, `GROUP BY main_hq_region` (see schema.md for the query). A direct `entities_iu.main_hq_region_unique_id` join is declared in dbt but not yet built |
