...
[Attach image / link to Loom / paste URL]
...
If any of Bug Description, Expected Behaviour, Steps to Reproduce, or Screenshot/Recording are missing, use `AskUserQuestion` to collect them before drafting.
...
Shown rendered (not wrapped in an outer fence) so the embedded mermaid diagrams display correctly. Copy the content below into Linear.

**Bug Description**

When an order status changes in the backend, the frontend dashboard doesn't update in real-time. Users see stale status until they manually refresh.
